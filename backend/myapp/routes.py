...
def hod_account_page(user_id):
    ...
    if len(leave_history) <= 5:
        ...
        latest_five = leave_history[:5]
    return {"success":1, 'username':user_id, 'role':'HOD', "latest_leaves":json.dumps(fun(latest_five))}
...
def authority_account_page(user_id):
    latest_five = []
    leave_history = db.session.query(Leave, ProcessedLeave).outerjoin(ProcessedLeave, Leave.id == ProcessedLeave.leave_id).filter(Leave.user_id == current_user.id).order_by(desc(Leave.applied_date),desc(Leave.applied_time)).all()
    if len(leave_history) <= 5:
        latest_five = leave_history
    else:
        latest_five = leave_history[:5]
    return {"success":1, 'user_id':user_id, 'role':'Authority', "latest_leaves":json.dumps(fun(latest_five))}
...
def administration_account_page(user_id):
    ...
    if len(leave_history) <= 5:
        ...
        latest_five = leave_history[:5]
    return {"success":1, 'user_id':user_id, 'role':'Administration', "latest_leaves":json.dumps(fun(latest_five))}
        ...
        if role == "HOD":
            if(action == 'insert'):
                if(leave_application.leave_category == 'Casual'):
                    ...
                    leave_to_insert = ProcessedLeave(leave_id = leave_id, hod_status = status)
...
def withdraw_leave_page(role, user_id, leave_id):
    try:
        db.query(Leave).filter(id == leave_id).delete()
        db.session.commit()
        return jsonify({"success_msg":1})
    except:
        return jsonify({"success_msg":0})
...
def view_user_page_for_admin(user_id, target_user):
    ...
    return {"user_id":user_id, "role":"Admin", "user":fun(user)}
...
def edit_user_page(user_id, target_user):
    ...
    return {"success":1, "user":fun(user)}
